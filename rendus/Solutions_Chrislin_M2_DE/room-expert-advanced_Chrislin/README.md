...
Row Key: APP_{id}_{date}_{timestamp}

2. Justifiez votre choix :

La distribution se fait naturellement par application et par date. Comme on a 1000 apps qui écrivent en même temps, les écritures sont réparties sur plusieurs régions HBase, donc pas de hotspot.
Le scan est super efficace pour la requête principale. Je peux faire un scan avec préfixe APP_001_20240106 et je récupère directement tous les logs de l'app 001 pour cette date.

3. Quels sont les avantages et inconvénients ?

Avantages :

Pas de risque de hotspot vu que 1000 applications différentes écrivent en parallèle sur différentes régions.
La requête principale est ultra rapide avec un simple scan préfixé, pas besoin de scanner toute la table.
La structure est lisible et facile à maintenir pour l'équipe. 

Inconvénients :

Pour la requête secondaire (tous les logs d'erreur du jour), je dois scanner toutes les apps, donc c'est lent et coûteux.
...
3 familles de colonnes

2. Comment les organisez-vous ?

CF1: metrics (CPU, RAM, disque)
CF2: events (logs, alertes)  
CF3: config (configuration)

3. Justifiez votre choix :
Je sépare les métriques parce qu'elles sont écrites toutes les 5 secondes donc beaucoup d'écritures. Comme ça je peux mettre un TTL court (7 jours) juste sur cette famille pour pas surcharger la table.
Les logs et alertes vont ensemble dans events parce qu'on les consulte souvent en même temps quand on cherche un problème. Fréquence d'écriture similaire aussi.
La config est toute seule car elle change presque jamais mais est lue souvent. Pattern complètement différent des autres, donc je la sépare pour pouvoir optimiser le cache dessus sans impacter les métriques.
...
Le problème c'est que toutes les sessions du même jour ont le même préfixe SESS_20240101_, donc elles tombent toutes dans la même région HBase. Toutes les écritures du jour vont sur une seule région alors que les autres régions ne font rien.

2. Quelle solution proposez-vous ?
Je propose du salting avec un préfixe calculé à partir de l'ID de session pour distribuer les écritures sur plusieurs régions.

3. Comment modifiez-vous les row keys ?
Row Key original: SESS_20240101_001
Row Key modifié:  3_SESS_20240101_001
Format: [hash(ID) % 10]_SESS_{date}_{id}

Je calcule un hash de l'ID modulo 10, ça me donne un chiffre entre 0 et 9. Les sessions sont maintenant distribuées sur 10 régions différentes au lieu d'être toutes dans la même.
...
PARTITIONED BY (date STRING, env STRING, app STRING)

2. Utilisez-vous des buckets ? Pourquoi ?
Non, pas besoin de buckets ici. Avec 50 apps × 2 environnements × 365 jours, j'ai déjà 36 500 partitions par an, c'est suffisant pour la distribution. Les buckets servent surtout pour les jointures et là on fait pas de jointure.

3. Justifiez votre choix :
Je partitionne d'abord par date parce que c'est la colonne la plus filtrée dans les requêtes. Après je mets l'environnement puis l'application parce que la requête fréquente filtre sur ces trois colonnes.
Pour la requête "Logs de l'app X en prod hier", Hive va directement dans la partition date=2024-01-05/env=prod/app=X sans scanner le reste. 
Pour la requête occasionnelle "Tous les logs d'erreur d'aujourd'hui", Hive scanne toutes les partitions de la date.
Je peux mettre un index sur la colonne "error" si ça devient vraiment fréquent.
...
SELECT * FROM ventes_textfile;


DROP TABLE ventes_textfile; 

3. Quels gains attendez-vous (espace, performance) ?
Espace : Je passe de 1 TB à environ 100-200 GB, donc je gagne 800-900 GB. ORC compresse entre 80-90% selon les données, donc j'économise du stockage.
Performance : Les requêtes seront 3 à 5 fois plus rapides grâce au format colonnaire. Si une requête prenait 10 minutes, elle prendra 2-3 minutes maintenant.
...
Partitionner la table par login_date
Filtrer sur status avant le GROUP BY
Créer une table agrégée si cette requête est fréquente
Utiliser format ORC pour lecture plus rapide 

2. Comment réécrivez-vous la requête ?

CREATE TABLE user_logins_daily AS
SELECT login_date, user_id, status, COUNT(*) as daily_count
FROM user_logins
GROUP BY login_date, user_id, status;

-- Requête optimisée
SELECT user_id, SUM(daily_count) as login_count
...
Avec le partitionnement par date, Hive ne scanne que les 365 partitions de 2024 au lieu de toute la table. Ça réduit déjà énormément le volume de données à lire.
Le format ORC lit seulement les colonnes nécessaires (user_id, status, login_date) au lieu de toute la ligne, donc gain de temps en lecture.
Si je crée la table agrégée, la requête sera plus rapide car les données sont déjà pré-groupées par jour.
...
Événements utilisateur
    ↓
    ├─→ HBase (Speed Layer) ─→ Recommandations temps réel
    │
    └─→ HDFS ─→ Hive (Batch Layer) ─→ Modèles ML entraînés
                ↓
            API de recommandations (Serving Layer)
            
2. Quelles données dans HBase ? Quelles données dans Hive ?
HBase (données chaudes < 24h):
Les clics et actions récentes de l'utilisateur, ses préférences actuelles, le panier en cours, les derniers produits vus. Tout ce qui change en temps réel et doit être accessible instantanément pour ajuster les recommandations.

Hive (données froides complètes):
L'historique complet des 2 ans d'achats, tous les clics passés, les patterns de comportement, les données démographiques. Tout ce qui sert à entraîner les modèles de ML et faire des analyses complexes.

3. Comment synchronisez-vous les deux systèmes ?
Écriture: Chaque événement utilisateur est écrit simultanément dans HBase (pour le temps réel) et dans HDFS (pour Hive). Kafka peut faire le routage des deux côtés.

Archivage: Un job Hive tourne chaque nuit pour récupérer les données de HBase vieilles de 24h et les archiver dans Hive. Après archivage, je supprime ces données de HBase pour libérer de l'espace.

Lecture: L'API de recommandations lit HBase pour les données fraîches et interroge Hive (via des vues pré-calculées) pour les patterns historiques. Elle combine les deux pour donner la recommandation finale.
...
HDFS Structure:
/datalake/landing/
    ├─ crm/          (données MySQL brutes)
    ├─ logs/         (fichiers texte bruts)
    └─ iot/          (fichiers JSON bruts)

/datalake/curated/
    ├─ crm/          (format ORC nettoyé)
    ├─ logs/         (format ORC parsé)
    └─ iot/          (format ORC structuré)

/datalake/analytics/
    └─ customer_360/ (données agrégées CRM+logs+IoT)

2. Comment intégrez-vous chaque source ?
CRM (MySQL): Sqoop extrait les données chaque nuit vers la landing zone. Un job Hive nettoie et convertit en ORC dans la curated zone. Je partitionne par date de modification.

Logs (texte): Flume ou script copie les logs vers landing. Job Hive parse les lignes, extrait les champs importants et stocke en ORC dans curated. Partitionnement par date de log.

IoT (JSON): Kafka ou NiFi ingère les événements vers landing. Job Hive utilise un SerDe JSON pour lire et convertir en ORC dans curated. Partitionnement par date et type de capteur.

3. Quel rôle joue Hive dans cette architecture ?
Hive sert d'interface SQL unique sur toutes les zones. Je crée des tables externes pointant vers landing pour accéder aux données brutes si besoin, et des tables managées dans curated et analytics.
Les équipes BI utilisent Hive pour faire des requêtes cross-sources sans savoir d'où viennent les données. Par exemple une requête joignant CRM + logs + IoT pour avoir une vue client complète.
Hive gère aussi les transformations entre zones avec des jobs schedulés qui déplacent et transforment les données de landing vers curated puis analytics. 
...
Application e-commerce
    ↓
Écritures → HBase (commandes, stock)
    ↓
Sync nocturne
    ↓
Lectures analytiques → Hive (rapports, tendances)
 

Les écritures (nouvelles commandes, mises à jour stock) vont directement dans HBase pour être rapides et atomiques. Les lectures analytiques (rapports mensuels, analyses de tendances) utilisent Hive avec des données dénormalisées et pré-agrégées.

2. Quelles données dans HBase ? Quelles données dans Hive ?
HBase :
Tables normalisées pour les commandes, le stock, les clients. Données fraîches et à jour en temps réel. Row keys optimisées pour les écritures fréquentes.

Hive :
Tables dénormalisées avec toutes les infos jointes: ventes_completes (commande + client + produit en une ligne), stock_historique, ventes_agregees (ventes par jour/mois/région pré-calculées). Format ORC partitionné par date.
3. Comment synchronisez-vous les deux modèles ?

Synchronisation quotidienne: Un job Hive tourne chaque nuit pour lire les nouvelles données de HBase (via une table externe Hive qui pointe vers HBase), faire les jointures et agrégations nécessaires, puis écrire dans les tables Hive optimisées.

Synchronisation en temps réel (optionnel): Pour certains rapports critiques, j'utilise Kafka ou Storm pour capturer les changements HBase en temps réel et mettre à jour Hive de façon incrémentale. Mais pour la plupart des analyses, le batch nocturne suffit
