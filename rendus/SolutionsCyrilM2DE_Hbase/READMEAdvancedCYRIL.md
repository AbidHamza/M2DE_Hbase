...
1. Quelle structure de row key proposez-vous ?
   RowKey = APP_ID#YYYYMMDD#(Long.MAX_VALUE - timestamp)#LOG_ID 

2. Justifiez votre choix :
   -Le LOG_ID garantit l’unicité.
   -La partition par date limite la taille des scans et facilite l’archivage.
   -Le préfixe APP_ID permet des scans efficaces par application.
   
3. Quels sont les avantages et inconvénients ?
   Avantages :
   -Bon compromis entre lisibilité et performance.
   -Scans efficaces pour : logs de l’application X le jour Y.
   
   Inconvénients : 
   -Row key plus longue (coût mémoire légèrement supérieur).
   -Risque de hotspot si une application génère énormément de logs → peut nécessiter du salting.
...
2. Comment les organisez-vous ?
   -CF_metrics     → CPU, RAM, disque (écriture très fréquente)
   -CF_logs        → logs applicatifs (append-only)
   -CF_metadata    → configuration, tags, seuils d’alerte


3. Justifiez votre choix :
   -Groupement par pattern d’accès et fréquence d’écriture.
   -Évite de charger logs + métriques lors d’une simple lecture de config.
...
1. Quel est le problème avec ces row keys ?
   -Row keys séquentielles (SESS_20240101_001, 002, 003)
   -Toutes les écritures arrivent dans la même région 

2. Quelle solution proposez-vous ?
   -Ajouter un salting ou un hash du préfixe 
...
2. Utilisez-vous des buckets ? Pourquoi ?
   -CLUSTERED BY (app_id) INTO 50 BUCKETS 

3. Justifiez votre choix :
   -date et environment sont toujours filtrés
   -Buckets :améliorent les jointures, équilibrent la lecture pour requêtes analytiques. 
...
3. Quels gains attendez-vous (espace, performance) ?
   -Espace : –70 à –90 %
   -Performance : x3 à x5 sur les requêtes analytiques
...
1. Quelles optimisations proposez-vous ?
   ...
   -Pré-agrégation journalière
   -Éviter le HAVING coûteux 

2. Comment réécrivez-vous la requête ?
   -SELECT user_id, SUM(cnt) AS login_count
...
3. Quels gains attendez-vous ?
   -Scan réduit drastiquement
   -Temps divisé par 5 à 10 
...
1. Comment organisez-vous l'architecture ?
   -HBase : données temps réel (<24h)
   -Hive : historique 2 ans 

2. Quelles données dans HBase ? Quelles données dans Hive ?
   -HBase : clics récents, préférences instantanées
   -Hive : historique d’achats, comportements long terme 

3. Comment synchronisez-vous les deux systèmes ?
   -Ingestion streaming (Kafka → HBase + HDFS)
   -Batch Hive quotidien
...
1. Comment organisez-vous le Data Lake ?
   -Landing Zone -> Curated Zone -> Analytics Zone 

2. Comment intégrez-vous chaque source ?
   -MySQL -> Sqoop 
   -Logs -> Ingestion brute
   -IoT JSON -> schéma Hive externe
   
3. Quel rôle joue Hive dans cette architecture ?
   -Métastore central
   -SQL unique
   -Jointures cross-sources 
...
2. Quelles données dans HBase ? Quelles données dans Hive ?
   ...
   -Hive -> ventes, tendances

3. Comment synchronisez-vous les deux modèles ?
   -Jobs batch incrémentaux 
...
2. Comment réécrivez-vous la requête ?
   -Réduction du volume avant jointure 

3. Quels paramètres ajustez-vous ?
   -SET hive.exec.parallel=true;
    SET hive.auto.convert.join=true;
    SET mapreduce.job.reduces=auto;
...
2. Comment structurez-vous les données dans HBase ?
   - Row keys : SERVER_ID#(inv_ts)
   - Familles de colonnes : metrics, meta
   - Justification : temps réel

3. Comment structurez-vous les données dans Hive ?
   - Partitions : month
   - Format de fichier : ORC
   - Justification : tendances long terme

4. Comment synchronisez-vous les deux systèmes ?
   -Streaming + batch quotidien 
