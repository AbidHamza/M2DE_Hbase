...
while [ $HIVE_CHECK_COUNT -lt $HIVE_MAX_CHECKS ]; do
    ...
    if echo "$HIVE_METASTORE_LOGS $HIVE_LOGS" | grep -qiE "(Cannot find hadoop installation|HADOOP_HOME.*not.*set|HADOOP_CONF_DIR.*not.*exist|Hadoop binaries not found|hadoop.*command not found|ERROR.*JAVA_HOME)" 2>/dev/null; then
        HIVE_ERRORS=1
